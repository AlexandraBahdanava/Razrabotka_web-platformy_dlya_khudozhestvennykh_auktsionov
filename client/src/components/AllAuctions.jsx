...
>
  {auctionData.map((auction) => (
    <Grid key={auction.id} item xs={1.6} md={2} lg={1.4}    margin={"0 40px"}>
     <Paper
      sx={{
        borderRadius: "16px",
        p: 0,
    ...
    >
      ...
      state: { idAuction: auction.id }
        ...
        <img src={auction.photo} alt="Auction" style={{objectFit: "cover", width: "260px", height: "220px", borderRadius: "8px" }} />
      <Box
            sx={{
              display: "flex",
...
<Typography variant="h1" > {`${auction.Artist.name}`}</Typography>
      <Typography variant="h1" > {`$${auction.starting_price}`}</Typography>  
          </Box>
</Link>
    </Paper>
    </Grid>
